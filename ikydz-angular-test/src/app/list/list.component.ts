...
export class ListComponent implements OnInit, OnDestroy {
  ...
  constructor(private service: DataService, private cdRef: ChangeDetectorRef,) { }

  ngOnInit() {

    this.service.currentCountryName.pipe(
      tap(console.log),
      takeUntil(this.onDestroy$),
    ).subscribe((name: string) => {
      this.searchCountriesList = this.countries.filter((country: string) => {
        return country.toLocaleLowerCase().startsWith(name.toLocaleLowerCase());
      });
  ...
  trackByFn(country: string): string {
    return country;
  }
